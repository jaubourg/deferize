...
function getPromiseField( type ) {
	return "_promise" + ( type ? ( "_" + type ) : "" );
}
...
function deferizeMethodFactory( internal ) {
	...
	return function( func, options ) {
		options = options || {};
		var promiseField = getPromiseField( options.type );
		return function() {
...
deferize.method.getter = function( field, type ) {
	return deferize.method.sync( function( callback ) {
		callback( this[ field ] );
	}, {
		type: type
	} );
};
deferize.method.fail = function( type ) {
	var promiseField = getPromiseField( type );
	return function() {
		var promise = this[ promiseField ];
		if ( promise ) {
